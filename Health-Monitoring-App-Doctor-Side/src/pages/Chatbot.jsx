...
const Chatbot = ({ chatId }) => {
  ...
  const handleSendMessage = () => {
    ...
    console.log("Chat ID:", 1); // Debugging
    console.log("User Message:", currentMessage); // Debugging
    ...
    fetch("http://localhost:5000/chat", {
      ...
      body: JSON.stringify({
        chat_id: chatId, // Ensure this value is passed correctly
  ...
  return (
    <div
      className={`flex flex-col h-screen ${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-900"} p-4`}
    >
      <div
      ...
      >
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => (
            <div
              key={index}
              className={`p-3 rounded-lg ${msg.role === "bot"
                  ? darkMode
            ...
            >
              ...
              {msg.role === "bot" ? (
                <ReactMarkdown>{msg.content}</ReactMarkdown> // Render bot messages with Markdown
              ) : (
                <p>{msg.content}</p>
              )}
            </div>
